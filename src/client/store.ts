...
export class NormalizerStore {
  ...
  public setStatusFilter = (
    filter: "all" | "healed" | "failed" | "direct",
  ): void => {
  ...
  private refreshAsync = async (): Promise<void> => {
    ...
    if (!adoptedFromFeed) {
      try {
        ...
        if (
          liveSnapshot.totalIntercepted > this.current.stats.totalIntercepted
        ) {
  ...
  public exportReport = (): void => {
    try {
      ...
      anchor.setAttribute(
        "download",
        `dsh_tool_normalizer_report_${new Date().toISOString().slice(0, 10)}.json`,
      );
