...
async function executeNestedTool(parent, tools, name2, args, suffix) {
  if (typeof tools.execute !== "function") {
    return recoveryFailure(
      `Cannot recover '${parent.name}': the host tools.execute() dispatcher is unavailable.`,
    );
  }
  if (parent.signal === void 0) {
    return recoveryFailure(
      `Cannot recover '${parent.name}': the original cancellation signal is unavailable.`,
    );
  }
  if (parent.token === void 0) {
    return recoveryFailure(
      `Cannot recover '${parent.name}': the original execution token is unavailable.`,
    );
  }
...
function isBridgeableDirectCall(exec, tools) {
  ...
  return (
    tools.get("run_code", exec.agent) !== void 0 &&
    tools.get(exec.name, exec.agent) !== void 0
  );
}
async function executeBridgeDirectCall(exec, tools) {
  if (typeof tools.execute !== "function") {
    return recoveryFailure(
      `Cannot recover '${exec.name}': the host tools.execute() dispatcher is unavailable.`,
    );
  }
  if (exec.signal === void 0) {
    return recoveryFailure(
      `Cannot recover '${exec.name}': the original cancellation signal is unavailable.`,
    );
  }
  if (exec.token === void 0) {
    return recoveryFailure(
      `Cannot recover '${exec.name}': the original execution token is unavailable.`,
    );
  }
  return executeNestedTool(
    exec,
    tools,
    exec.name,
    exec.arguments ?? {},
...
function injectInnerDescriptions(
  code,
  outerDescription,
  ...
  while (i < code.length) {
    ...
    if (ch === "t" && code.startsWith("tools.", i)) {
      ...
      if (match !== null) {
        ...
        if (
          hasDescriptionProperty(code, open, close) ||
          !requiresDescription(match[1])
        ) {
...
function normalizeEditorArguments(
  toolName,
  rawArgs,
  cwd = process.cwd(),
  maxLines,
  ...
  const args = { ...rawArgs };
  const pathKey = ["path", "file_path", "TargetFile"].find(
...
function normalizeRunCodeArguments(rawArgs) {
  ...
  if (
    ...
    (!argsObj["code"] || typeof argsObj["code"] !== "string")
  ) {
    const cmdStr = rawCommand.trim();
    const description2 =
      typeof argsObj["description"] === "string" &&
      argsObj["description"].trim().length > 0
...
function isRegexStart2(code, start) {
  ...
  if (/[A-Za-z0-9_$]/.test(previous)) {
    let wordStart = i;
    while (wordStart >= 0 && /[A-Za-z0-9_$]/.test(code[wordStart])) wordStart--;
    const word = code.slice(wordStart + 1, i + 1);
    return /* @__PURE__ */ new Set([
      "return",
      "throw",
...
function registerPromptGuidance(ctx) {
  const systemPrompt =
    typeof ctx.get === "function" ? ctx.get("systemPrompt") : ctx.systemPrompt;
  ...
  if (typeof ctx.effect === "function") {
    ctx.effect(
      ...
      "tool-normalizer: prompt guidance",
    );
  } else {
    systemPrompt.section(section);
  }
...
function validRecord(value) {
  ...
  if (
    value.normalizedArgsPreview !== void 0 &&
    typeof value.normalizedArgsPreview !== "string"
  )
    return false;
  if (
    value.normalizationSummary !== void 0 &&
...
function healingRate(stats) {
  ...
  return attempts > 0
    ? Math.round((stats.healedSuccess / attempts) * 1e3) / 10
    : 0;
}
...
async function observeAndRetryMutation(exec, tools) {
  ...
  if (tools.get("read", exec.agent) !== void 0) {
    ...
    tools.get("str_replace_editor", exec.agent) !== void 0
  ) {
...
function readTokenMeter(ctx) {
  ...
  return meter !== void 0 && typeof meter.measure === "function"
    ? meter
    : void 0;
}
...
function apply(ctx, userConfig = {}) {
  ...
  ctx.on("tools/execute", async (exec, next) => {
    ...
    const savedTokens = (healed, ok, category) =>
      healed && ok
    ...
    if (exec.name === "run_code" && config.autoWrapRunCode) {
      ...
      const hasMarkdownFence =
        rawCode !== void 0 && stripMarkdownFences(rawCode) !== rawCode;
      ...
      if (isCmdPass)
        changes.push("\u5C06 command/cmd \u8F6C\u4E3A run_code.code");
      if (isMissingDesc) changes.push("\u8865\u5168 run_code.description");
      if (hasMarkdownFence)
        changes.push("\u79FB\u9664 code \u7684 Markdown \u56F4\u680F");
      ...
      const codeBody =
        typeof normalized.code === "string" ? normalized.code : void 0;
      if (codeBody !== void 0) {
        ...
        if (inner.injected > 0) {
          ...
          changes.push(
            `\u8865\u5168\u5185\u5C42 description \xD7 ${inner.injected}`,
          );
    ...
    try {
      ...
      if (
        ...
        if (lineCount !== void 0) {
          ...
          if (retried !== void 0) {
            ...
            changes.push(
              `\u6309\u6587\u4EF6\u771F\u5B9E\u884C\u6570\u4FEE\u6B63 view_range\uFF08${lineCount} \u884C\uFF09`,
            );
      ...
      recordEvent({
        ...
        normalizationSummary:
          changes.length > 0 ? changes.join("\uFF1B") : void 0,
