...
function normalizeEditorArguments(toolName, rawArgs, cwd = process.cwd(), maxLines, resolveRelativePaths = true) {
...
var OFFSET_TOOLS = /* @__PURE__ */ new Set(["read", "glob", "grep"]);
function normalizeReadArguments(toolName, rawArgs) {
  if (!rawArgs || typeof rawArgs !== "object") return void 0;
  if (!OFFSET_TOOLS.has(toolName)) return void 0;
  const args = { ...rawArgs };
  let changed = false;
  const offsetKey = "offset" in args ? "offset" : "start" in args ? "start" : void 0;
  if (offsetKey !== void 0) {
...
// src/normalizers/run-code-syntax.ts
var BACKTICK2 = String.fromCharCode(96);
...
function skipLineComment2(code, start) {
  const newline = code.indexOf("\n", start + 2);
  return newline === -1 ? code.length : newline;
}
function skipBlockComment2(code, start) {
  const end = code.indexOf("*/", start + 2);
  return end === -1 ? -1 : end + 2;
}
function isRegexStart2(code, start) {
  let i = start - 1;
  while (i >= 0 && /\s/.test(code[i])) i--;
  if (i < 0) return true;
  const previous = code[i];
...
function skipRegex2(code, start) {
  let inClass = false;
  let i = start + 1;
  while (i < code.length) {
    const ch = code[i];
...
function skipStringOrTemplate2(code, start) {
  const quote = code[start];
  let i = start + 1;
  while (i < code.length) {
    const ch = code[i];
    ...
    if (ch === "$" && code[i + 1] === "{") {
      const end = skipTemplateExpression2(code, i + 2);
  ...
  return { unterminated: quote === BACKTICK2 ? "template" : "string" };
}
function skipTemplateExpression2(code, start) {
  let depth = 1;
  let i = start;
  while (i < code.length) {
    const ch = code[i];
    if (ch === String.fromCharCode(39) || ch === String.fromCharCode(34) || ch === BACKTICK2) {
      const end = skipStringOrTemplate2(code, i);
      if (!("end" in end)) return -1;
      i = end.end;
      continue;
    }
    if (ch === "/" && code[i + 1] === "/") {
      i = skipLineComment2(code, i);
      continue;
    }
    if (ch === "/" && code[i + 1] === "*") {
      const end = skipBlockComment2(code, i);
      if (end < 0) return -1;
      i = end;
      continue;
    }
    if (ch === "/" && code[i + 1] !== "/" && code[i + 1] !== "*" && isRegexStart2(code, i)) {
      const end = skipRegex2(code, i);
      if (!("end" in end)) return -1;
      i = end.end;
      continue;
    }
...
function walkStructure(code, at) {
  let i = 0;
  while (i < code.length) {
    const ch = code[i];
    if (ch === String.fromCharCode(39) || ch === String.fromCharCode(34) || ch === BACKTICK2) {
      const end = skipStringOrTemplate2(code, i);
      if (!("end" in end)) {
        const stopped2 = at("\0", i);
        return { stoppedAt: stopped2 ?? i };
      }
      i = end.end;
      continue;
    }
    ...
    if (ch === "/" && code[i + 1] !== "/" && code[i + 1] !== "*" && isRegexStart2(code, i)) {
      const end = skipRegex2(code, i);
      if (!("end" in end)) return void 0;
      i = end.end;
      continue;
    }
    const stopped = at(ch, i);
    if (stopped !== void 0) return { stoppedAt: stopped };
    i++;
  }
...
function missingTailClosers(code) {
  const stack = [];
  let stoppedAt = -1;
  ...
  const walk = walkStructure(code, (ch, index) => {
    ...
    if (ch === ")" || ch === "}" || ch === "]") {
      ...
      if (open === void 0 || OPEN_CLOSERS[open] !== ch) {
        stoppedAt = index;
        return index;
      }
      return void 0;
  ...
  if (walk === void 0 || stoppedAt >= 0) return void 0;
  if (stack.length === 0) return void 0;
  if (unterminatedNode && stack.length === 1) return void 0;
  const closers = [];
  for (let i = stack.length - 1; i >= 0; i--)
    closers.push(OPEN_CLOSERS[stack[i]]);
  return closers.join("");
}
function countQuotesFrom(code, start) {
  const quote = code[start];
  let count = 0;
...
function repairTruncatedTail(code) {
  const tail = code.replace(/\s+$/u, "");
  const lastQuote = Math.max(
    tail.lastIndexOf(String.fromCharCode(34)),
    tail.lastIndexOf(String.fromCharCode(39)),
    tail.lastIndexOf(BACKTICK2)
  );
  if (lastQuote < 0) return void 0;
  const scanned = skipStringOrTemplate2(tail, lastQuote);
  ...
  if (isUnterminated && countQuotesFrom(tail, lastQuote) % 2 === 1) {
    ...
    if (balanced === void 0) return void 0;
    const candidate = closed + balanced;
    return parseProgram(candidate) ? candidate : void 0;
  }
  return void 0;
}
function rewriteTripleQuotedStrings(code) {
  const spans = [];
  let i = 0;
  while (i < code.length) {
    const ch = code[i];
    if (ch === String.fromCharCode(39) || ch === String.fromCharCode(34)) {
      ...
      const scanned = skipStringOrTemplate2(code, i);
      if (!("end" in scanned)) return void 0;
      i = scanned.end;
      continue;
    }
    if (ch === BACKTICK2) {
    ...
    if (ch === "/" && code[i + 1] !== "/" && code[i + 1] !== "*" && isRegexStart2(code, i)) {
      const scanned = skipRegex2(code, i);
      if (!("end" in scanned)) return void 0;
      i = scanned.end;
      continue;
    }
  ...
  return parseProgram(out) ? out : void 0;
}
function isEscapedBacktick(code, index) {
  let backslashes = 0;
  for (let i = index - 1; i >= 0 && code[i] === "\\"; i--) backslashes++;
  return backslashes % 2 === 1;
}
function escapeStrayTemplateBackticks(code) {
  const backticks = [];
  for (let i = 0; i < code.length && backticks.length <= 256; i++) {
    if (code[i] === BACKTICK2 && !isEscapedBacktick(code, i)) backticks.push(i);
  }
  const opener = backticks[0];
  if (opener === void 0 || backticks.length < 3) return void 0;
...
function escapeBackticksExcept(code, backticks, kept) {
  let out = "";
  let cursor = 0;
  for (const index of backticks) {
    if (kept.has(index)) continue;
    out += `${code.slice(cursor, index)}\\${BACKTICK2}`;
    cursor = index + 1;
  }
  out += code.slice(cursor);
  return out;
}
function repairRunCodeSyntax(code) {
  if (parseProgram(code)) return void 0;
  const repairers = [
    repairTruncatedTail,
  ...
  for (const repairer of repairers) {
    ...
    if (candidate !== void 0 && candidate !== code) return candidate;
  }
  return void 0;
}
...
var TOOL_NORMALIZER_TOP_ERRORS_SECTION = "tool-normalizer:top-errors";
var DEFAULT_GUIDANCE_HEADING = "## Tool Call Reliability & Best Practices";
var DEFAULT_GUIDANCE_TEXT = `${DEFAULT_GUIDANCE_HEADING}
...
var currentGuidanceText = DEFAULT_GUIDANCE_TEXT;
var currentTopErrorsText = "";
...
function resetGuidanceText() {
  currentGuidanceText = DEFAULT_GUIDANCE_TEXT;
}
function getTopErrorsText() {
  return currentTopErrorsText;
}
function refreshTopErrors(tracker) {
  const stats = tracker.getAggregate();
  if (stats.totalIntercepted === 0) {
    currentTopErrorsText = "";
    return;
  }
  const toolFailures = Object.entries(stats.failuresByCategory).filter(([cat]) => cat !== "PASSTHROUGH").map(([cat, count]) => ({ cat, count })).filter((e) => e.count >= 2).sort((a, b) => b.count - a.count).slice(0, 3).map((e) => `  - ${e.cat}: ${e.count} occurrences`);
  const toolCallErrors = Object.entries(stats.failuresByTool).filter(([tool]) => tool !== "run_code").map(([tool, count]) => ({ tool, count })).filter((e) => e.count >= 2).sort((a, b) => b.count - a.count).slice(0, 3).map((e) => `  - ${e.tool}: ${e.count} failed calls`);
  const parts = [];
...
var LOG_SIZE_CAP_BYTES = 2 * 1024 * 1024;
var LOG_COMPACT_KEEP_BYTES = 1024 * 1024;
var LOG_SIZE_CHECK_EVERY_APPENDS = 128;
...
function validRecord(value) {
  ...
  if (value.normalizedArgsPreview !== void 0 && typeof value.normalizedArgsPreview !== "string")
    return false;
  if (value.normalizationSummary !== void 0 && typeof value.normalizationSummary !== "string")
    return false;
  if (value.errorMessage !== void 0 && typeof value.errorMessage !== "string")
    return false;
  if (value.tokensSaved !== void 0 && !nonNegativeCount(value.tokensSaved))
    return false;
  return true;
}
...
function accumulateRecord(totals, byTool, byCategory, failuresByTool, failuresByCategory, record) {
  ...
  if (record.status === "failed") {
    ...
    failuresByCategory[record.category] = (failuresByCategory[record.category] ?? 0) + 1;
  }
}
...
function aggregateFromRecords(records) {
  ...
  const failuresByTool = /* @__PURE__ */ Object.create(
    null
  );
  const failuresByCategory = /* @__PURE__ */ Object.create(
    null
  );
...
function scheduleSummary(stats) {
...
async function compactLogIfOversized() {
  if (isTestRun()) return;
  let size;
  try {
...
function runCodeArgsMatch(rawArgs, normalized) {
  ...
  if (typeof rawObject["code"] !== "string" || typeof rawObject["description"] !== "string")
    return false;
...
function isSelfNestedExec(exec) {
  return typeof exec.callId === "string" && exec.callId.includes(":normalizer:");
}
function healMatchesError(category, errorCode2, errorMessage) {
  ...
  switch (category) {
    ...
    case "CODE_WRAP":
    case "READ_ARGS":
      return errorCode2 === "INVALID_ARGS" || /offset|start|limit/i.test(text);
    case "INNER_DESC":
      return errorCode2 === "INVALID_ARGS" || /required property|invalid arguments|description/i.test(text);
    case "RUN_CODE_SYNTAX":
      return isSyntaxLikeRunFailure(errorCode2, text);
    case "RANGE_CLAMP":
      return /absolute path|view_range|view range|number of lines|out of/i.test(
        text
      ) || errorCode2 === "FS_NOT_FOUND";
  }
}
function isSyntaxLikeRunFailure(errorCode2, errorMessage) {
  if (errorCode2 !== "CODE_RUN_FAILED") return false;
...
function collapsedCallHint(toolName) {
  return `[tool-normalizer hint] Direct '${toolName}' calls are hidden in Code-Mode; reissue it inside run_code instead: run_code({ code: "const r = await tools.${toolName}(<args>); return r;", description: "<what this call does>" }).`;
}
var SYNTAX_HINT = "[tool-normalizer hint] The run_code program failed to parse before running. Usual causes: unescaped backticks inside template literals, Python pasted as JS (`def`/`print`/`'''`), or a truncated tail. Keep the program short; write long shell/python bodies to a file and run that file instead.";
function appendResultHint(result, hint) {
...
function resultStatus(result, wasHealed) {
...
function lineCountFromRangeError(result) {
  ...
  if (message === void 0 || !message.includes("view_range"))
    return void 0;
  const match = /(?:range of lines of the file:\s*\[1,\s*|number of lines in the file:\s*`?)(\d+)/u.exec(
    message
  );
...
async function observeAndRetryMutation(exec, tools) {
  ...
  let readName;
  if (tools.get("read", exec.agent) !== void 0) {
    readName = "read";
  } else if (exec.name === "str_replace_editor" && tools.get("str_replace_editor", exec.agent) !== void 0) {
...
async function refreshObservation(exec, tools) {
...
function readCodeRuntimeLanguage(ctx) {
  const runtime = typeof ctx.get === "function" ? ctx.get("codeRuntime") : ctx.codeRuntime;
  return typeof runtime?.language === "string" ? runtime.language : void 0;
}
function avoidedRoundTrips(category) {
  if (category === "FS_OBSERVED") return 2;
  if (category === "UNKNOWN_TOOL" || category === "INVALID_ARGS" || category === "RANGE_CLAMP" || category === "CODE_WRAP" || category === "RUN_CODE_DESC" || category === "RUN_CODE_SYNTAX" || category === "INNER_DESC" || category === "READ_ARGS")
    return 1;
  return 0;
}
function readTokenMeter(ctx) {
  const meter = typeof ctx.get === "function" ? ctx.get("tokenMeter") : ctx.tokenMeter;
  return meter !== void 0 && typeof meter.measure === "function" ? meter : void 0;
}
function measureTokensSaved(meter, agent, roundTrips) {
  if (roundTrips <= 0 || meter === void 0) return 0;
...
function apply(ctx, userConfig = {}) {
  ...
  const pendingHeals = /* @__PURE__ */ new Map();
  const MAX_PENDING_HEALS = 1e3;
  const inlineRecorded = /* @__PURE__ */ new Set();
  const MAX_INLINE_RECORDED = 2e3;
  ...
  function buildRecord(pending, result, agent) {
    const ok = !result.isError;
    return {
      ...pending,
      status: resultStatus(result, pending.wasHealed),
      errorMessage: resultErrorText(result),
      tokensSaved: pending.wasHealed && ok ? measureTokensSaved(
        readTokenMeter(ctx),
        agent,
  ...
  function stashPending(callId, pending, outcome, agent) {
    if (callId === void 0 || resultFallback) {
    ...
    if (!resultHookConfirmed && fallbackTimer === void 0) {
      fallbackTimer = setInterval(sweepUnobservedHeals, 15e3);
      fallbackTimer.unref?.();
    }
  }
  function recordInline(callId, pending, outcome) {
    if (callId !== void 0) {
  ...
  function sweepUnobservedHeals() {
    ...
    for (const { pending, outcome } of backlog) {
      recordInline(void 0, pending, outcome);
    }
  }
  ctx.logger?.info?.(
  ...
  ctx.on(
    ...
    async (exec, next) => {
      ...
      let rawArgsStr;
      const getRawArgsStr = () => rawArgsStr ??= JSON.stringify(originalArgs ?? {}) ?? "{}";
      const getOriginalPreview = (needed) => needed ? compactPreview(getRawArgsStr()) : "";
      const startTime = Date.now();
      const eventId = `norm_${startTime}_${Math.random().toString(36).slice(2, 8)}`;
      const tools = getTools();
      const meter = getMeter();
      const savedTokens = (healed, ok, category) => healed && ok ? measureTokensSaved(meter, exec.agent, avoidedRoundTrips(category)) : 0;
      let wasHealed = false;
      let healCategory = "PASSTHROUGH";
      let normalizedPreview;
      const changes = [];
      let isPythonProgram = false;
      const currentPending = (previewNeeded) => ({
        id: eventId,
        time: startTime,
        toolName: exec.name,
      ...
      if (exec.name === "run_code" && config.autoWrapRunCode) {
        ...
        if (normalized.code.trim().length === 0) {
          ...
          if (codeBody !== void 0 && !isPythonProgram) {
            ...
            const inner = injectInnerDescriptions(
              ...
              (toolName) => {
                ...
                const required = tools !== void 0 && toolRequiresDescription(tools, toolName, exec.agent);
      ...
      if ((exec.name === "read" || exec.name === "glob" || exec.name === "grep") && config.autoClampRanges) {
      ...
      try {
        ...
        if (config.autoObserveFiles && !isObservationRetry(exec) && tools && (resultHasCode(result, "FS_NOT_OBSERVED") || resultHasCode(result, "FS_STALE_VERSION"))) {
        ...
        if (config.autoObserveFiles && !isSelfNestedExec(exec) && tools && result.isError === true && (resultHasCode(result, "FS_EDIT_NOT_FOUND") || resultHasCode(result, "FS_AMBIGUOUS_EDIT"))) {
        ...
        if (result.isError === true && wasHealed && healCategory !== "PASSTHROUGH" && healCategory !== "FS_OBSERVED" && healCategory !== "RANGE_CLAMP" && !healMatchesError(
          healCategory,
          errorCode(result.isError ? result.error : void 0),
        ...
        if (config.errorHints && result.isError === true) {
          ...
          if (resultHasCode(result, "UNKNOWN_TOOL") && failureText.includes("only `run_code` is callable directly")) {
            result = appendResultHint(result, collapsedCallHint(exec.name));
          } else if (exec.name === "run_code" && !isPythonProgram && healCategory !== "RUN_CODE_SYNTAX" && isSyntaxLikeRunFailure(
            errorCode(result.isError ? result.error : void 0),
            failureText
          )) {
  ...
  ctx.on(
    ...
    (exec, result) => {
      try {
        if (typeof exec.callId === "string" && exec.callId.includes(":normalizer:")) {
          return void 0;
        }
        resultHookConfirmed = true;
        const callId = exec.callId;
        const stashed = typeof callId === "string" ? pendingHeals.get(callId) : void 0;
        if (stashed !== void 0 && typeof callId === "string") {
          pendingHeals.delete(callId);
        }
        const alreadyInline = stashed === void 0 && typeof callId === "string" && inlineRecorded.delete(callId);
...
export {
  DEFAULT_GUIDANCE_HEADING,
  DEFAULT_GUIDANCE_TEXT,
  NORMALIZER_CATEGORIES,
  TOOL_NORMALIZER_PROMPT_SECTION,
  TOOL_NORMALIZER_TOP_ERRORS_SECTION,
