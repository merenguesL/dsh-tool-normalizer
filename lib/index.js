...
function refreshTopErrors(tracker) {
  ...
  const toolFailures = Object.entries(stats.byCategory).filter(([cat]) => cat !== "PASSTHROUGH" && cat !== "READ_ARGS").map(([cat, count]) => ({ cat, count })).filter((e) => e.count >= 2).sort((a, b) => b.count - a.count).slice(0, 3).map((e) => `  - ${e.cat}: ${e.count} occurrences`);
  const toolCallErrors = Object.entries(stats.byTool).filter(([tool]) => tool !== "run_code").map(([tool, count]) => ({ tool, count })).filter((e) => e.count >= 2).sort((a, b) => b.count - a.count).slice(0, 3).map((e) => `  - ${e.tool}: ${e.count} failed calls`);
  const parts = [];
...
function apply(ctx, userConfig = {}) {
  ...
  function buildRecord(pending, result) {
  ...
  function stashPending(callId, pending, outcome) {
