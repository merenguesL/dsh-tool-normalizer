...
function findObjectEnd(code, objStart) {
  let depth = 0;
  let i = objStart;
  while (i < code.length) {
    const ch = code[i];
...
var TOOLS_CALL = /tools\.([A-Za-z_$][\w$]*)\s*\(\s*\{/g;
function injectInnerDescriptions(code, outerDescription) {
  const out = [];
  let cursor = 0;
  let injected = 0;
  ...
  for (let m = TOOLS_CALL.exec(code); m !== null; m = TOOLS_CALL.exec(code)) {
    ...
    if (/\bdescription\s*:/.test(literal)) {
      ...
      const label = `${outerDescription || "inner call"} \xB7 ${m[1]}`.slice(0, 80);
