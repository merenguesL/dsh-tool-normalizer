...
var timer;
function persistSnapshotSoon(tracker) {
  clearTimeout(timer);
  timer = setTimeout(() => {
    void mkdir(join(statsFilePath(), ".."), { recursive: true }).then(() => writeFile(statsFilePath(), JSON.stringify(tracker.getSnapshot(), null, 2))).catch(() => {
    });
  }, 1e3);
  timer.unref?.();
}
